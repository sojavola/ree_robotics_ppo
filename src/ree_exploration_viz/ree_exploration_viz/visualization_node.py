...
class REEVisualizationNode(Node):
    ...
    def __init__(self):
        ...
        # Publier la grille et la legende au demarrage (one-shot — détruit dans le callback)
        self._initial_timer = self.create_timer(1.0, self.initial_publish)
    ...
    def initial_publish(self):
        """Publication initiale (one-shot)."""
        self.publish_reference_grid()
        self.publish_legend()
        self.destroy_timer(self._initial_timer)
